...
function createTables(){// função responsavel por criar as tabelas.
    var query1='CREATE TABLE IF NOT EXISTS PRODUTO (ID_PRODUTO INTEGER  NOT NULL PRIMARY KEY AUTOINCREMENT,NOME_PRODUTO VARCHAR(200) NOT NULL UNIQUE,DIAS_PRODUCAO VARCHAR(3) NOT NULL);';
...
function dropTables(){// função responsavel por dropar as tabelas do banco de dados.
    var query = 'DROP TABLE AGENDAMENTO;';
    try {
        localDB.transaction(function(transaction){// Toda vez que for execurtar o código sql é necessário criar uma transação.
...
function deleteDados(){// função responsavel por dropar as tabelas do banco de dados.
    var query = 'delete from produto where ID_PRODUTO=1;';
    try {
        localDB.transaction(function(transaction){// Toda vez que for execurtar o código sql é necessário criar uma transação.
            transaction.executeSql(query, [], nullDataHandler, errorHandler);
...
function insertDados(){// função responsavel por inserir dados na tabela.
    ...
    var query = "insert into PRODUTO(NOME_PRODUTO,DIAS_PRODUCAO) values(?,?);";
        try {
        localDB.transaction(function(transaction){// Toda vez que for execurtar o código sql é necessário criar uma transação.
...
function selecionaDados(){
    ...
    var query = "SELECT * FROM PRODUTO;";// Código sql
    try {
        var result = [];
        localDB.transaction(function(transaction){
            ...
            transaction.executeSql(query, [], function(transaction, results){// Toda vez que for execurtar o código sql é necessário criar uma transação.
                for (var i = 0; i < results.rows.length; i++) {
                   ...
                   // console.log(row['ID_PRODUTO']+" "+row['NOME_PRODUTO']+" "+row['DIAS_PRODUCAO']);//row['idade'] variável responsavel por 
                    var lp = new_product(row['ID_PRODUTO'],row['NOME_PRODUTO'],row['DIAS_PRODUCAO']);
                    result.push(lp);
